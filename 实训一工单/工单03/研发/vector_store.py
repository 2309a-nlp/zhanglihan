...
class VectorStore:
    ...
    def __init__(self, model_name: str = None):
        ...
        # ── 线程锁 ──
        self._lock = threading.Lock()    # 保护 FAISS index 写入
        # 注意：FAISS search() 本身是读安全的，但 embedding model 需要锁
        self._embed_lock = threading.Lock()  # 嵌入模型锁（单线程推理）
    ...
    def search(self, query: str, top_k: int = TOP_K_RESULTS
        ...
        两路分数分别归一化后加权融合
        线程安全：FAISS search() 读安全，sentence-transformers encode() 加锁
        """
    ...
    def embed_texts(self, texts: List[str]) -> np.ndarray:
        """sentence-transformers 本地推理，L2 归一化（线程安全）"""
