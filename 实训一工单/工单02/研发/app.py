# -*- coding: utf-8 -*-
"""
工单02 - 智能对话问答系统（Streamlit 界面 | 速度优化版）
全链路优化目标：首次问答 < 3s
关键优化：
...
# ── 清除 PYTHONPATH 中的 Python312 site-packages（与本环境不兼容）──
import os, sys
pp = os.environ.get("PYTHONPATH", "")
if pp:
    clean = [p for p in pp.split(";") if "Python312" not in p]
    os.environ["PYTHONPATH"] = ";".join(clean)
    sys.path[:] = [p for p in sys.path if "Python312" not in p]
...
def main():
    ...
    with st.sidebar:
        st.title("⚡ 工单02 问答系统")
        st.caption("全链路优化 · < 3s · 高并发稳定 · bge-small-zh-v1.5 · DeepSeek")
